...
function withConstructorInput(name: string, arg: Argument): ArtifactMapper {
  return (artifact: Artifact): Artifact => {
    ...
    const constructorInputToRemove = artifact.constructorInputs.find(p => p.name === name);
    ...
    return {
      ...
      constructorInputs: artifact.constructorInputs.filter(p => p.name !== name),
      functions: artifact.functions.map(replaceASMtoken(name, argEncoded.toString('hex'))),
    };
  }
}

function withCustomConstructorInputName(toreplace: string, replaceby: string): ArtifactMapper {
  return (artifact: Artifact): Artifact => {
      return {
        ...artifact,
        constructorInputs: artifact.constructorInputs.map(p => p.name === toreplace ? { ...p, name: replaceby } : p),
        functions: artifact.functions.map(replaceASMtoken(toreplace, `$${replaceby}`)),
      }
  }
}

function replaceASMtoken(toreplace: string, by: string): (f: Function) => Function {
...
function transformArtifact(artifact: Artifact, ...mappers: ArtifactMapper[]): Artifact {
